...
final case class HeatMap[T: Numeric](parent: FramePanel,
  ...
  override def redraw(focus: Boolean, theme: ColorScheme): Unit = {
    ...
    val graphWidth   = (if showLabels then innerWidth - 2 else innerWidth) - valuesLength
...
//      showVertical = false, showHorizontal = false)
    ...
    val max = array.max
    for x <- 0 until graphWidth; y <- 0 to dh by 2 do {
      val v1 = array(x, y)
      val v2 = if y != dh + 1 then array(x, y + 1) else -1
      val c1 = Palettes.mapToRGB(v1, max, quadratic = true)
      val c2 = if v2 != -1 then Palettes.mapToRGB(v2, max, quadratic = true) else -1
