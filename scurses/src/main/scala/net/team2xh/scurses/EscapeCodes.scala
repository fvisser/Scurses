...
class EscapeCodes(out: OutputStream) {
  ...
  private def ESC(command: Char): Unit = { out.write(s"\u001b$command".getBytes) }
  // Output a Control Sequence Inroducer
  private def CSI(sequence: String): Unit = { out.write(s"\u001b[$sequence".getBytes) }
  // Execute commands
  ...
  def getReport(csi: () => Unit, args: Int, terminator: Char): Array[Int] = {
    ...
    System.in.read()
    // Parse each Ps
    for i <- 0 until args do {
      var n = System.in.read()
      while n != separators(i).toInt do {
        results(i) += n.toChar
        n = System.in.read()
      }
    }
